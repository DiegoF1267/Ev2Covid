...
public class PacientesDAOSQLite implements PacientesDAO {
    ...
    public  PacientesDAOSQLite(Context context){
        this.pacHelper= new PacientesSQLiteHelpers(context,"DBPacientes",null,1);
    }
    ...
    public List<Pacientes> getAll() {
        ...
        try {
            if(reader != null){
                Cursor c = reader.rawQuery("SELECT rut,nombre,apellido,fecha,area,covid,temperatura,tos,presion"+
                        "FROM pacientes",null);
                if (c.moveToFirst()){
                    do{
    ...
    public Pacientes save(Pacientes p) {
        ...
        String sql=String.format("INSERT INTO pacientes(rut,nombre,apellido,fecha,area,covid,temperatura,tos,presion)"+
                " VALUES ('%s','%s','%s','%s','%s','%s',%d,'%s',%d)", p.getRut(), p.getNombre(), p.getApellido()
